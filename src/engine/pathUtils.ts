...
export function stringifySvgPath(commands: PathCommand[]): string {
  return commands.map(c => `${c.type}${c.args.join(' ')}`).join(' ');
}

export function absolutizePath(commands: PathCommand[]): PathCommand[] {
  let cx = 0, cy = 0;
  let startX = 0, startY = 0;
  
  const absCommands: PathCommand[] = [];
  ...
  for (const cmd of commands) {
    ...
    let pairCount = 2;
    if (upper === 'Q' || upper === 'S') pairCount = 4;
    if (upper === 'C') pairCount = 6;
    if (upper === 'A') pairCount = 7;
    ...
    for (let i = 0; i < args.length; i += pairCount) {
      ...
      if (chunk.length < pairCount) break; // invalid path chunk
...
// Extract a flat list of node objects containing a reference to their command and index
...
export function extractNodes(commands: PathCommand[]): PathNodeRef[] {
  const nodes: PathNodeRef[] = [];
