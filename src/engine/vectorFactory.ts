...
import type { XcsDisplay, XcsCharJSON, ElementStyle } from './types';
 ...
 */
export function parseColor(value: string | number | undefined | null): string {
 ...
 */
export function resolveStyle(el: XcsDisplay): ElementStyle {
 ...
 */
function styleToAttrs(style: ElementStyle): string {
  const parts: string[] = [];
...
export class VectorFactory {
  ...
  static generateElement(
    ...
    if (type === '2d') {
      if (el.base64) type = 'BITMAP';
      else if (el.dPath || el.points) type = 'PATH';
    }
   ...
   */
  static getPathData(el: XcsDisplay, transform: Transform): { d: string, attrs: string, transform: string } | null {
    let type = el.type || (el.base64 ? 'BITMAP' : 'UNKNOWN');
    if (type === '2d' && !el.base64 && (el.dPath || el.points)) type = 'PATH';
  ...
  static generateRect(el: XcsDisplay, transform: Transform): string {
    ...
    const ry = (el as any).cornerRadius ?? 0;
    ...
    if (el.fontData?.glyphData) {
      ...
      for (const [, glyph] of Object.entries(glyphs)) {
        if (glyph.dPath) {
          ...
          paths.push(
            `<path d="${glyph.dPath}" transform="${charTransform.toSVG()}" />`
          );
   ...
   */
  private static generateCharPath(char: XcsCharJSON, parent: XcsDisplay): string {
    const d = char.dPath || (char as any).glyphData || '';
    if (!d.trim()) return '';

    // Each glyph can have its own style in XCS
    const charStyle = resolveStyle(char as any);
    ...
    const charTransform = Transform.fromXcsElement({
      x: char.graphicX ?? char.x ?? (char as any).offsetX ?? (char as any).dx,
      y: char.graphicY ?? char.y ?? (char as any).offsetY ?? (char as any).dy,
      angle: char.angle,
      scale: scale,
      skew: char.skew,
      pivot: char.pivot,
  ...
  static generatePen(el: XcsDisplay, transform: Transform): string {
    ...
    if (points.length === 0) {
      ...
      if (el.dPath) return VectorFactory.generatePath(el, transform);
      return '';
    }
  ...
  static generateClipPath(el: XcsDisplay, id: string): string {
    ...
    switch (type) {
      ...
      case 'RECT':
        ...
        break;
      default:
        if (el.dPath) inner = `<path d="${el.dPath}" />`;
        break;
    }
  ...
  static generateImage(el: XcsDisplay, transform: Transform): string {
    ...
    const href = el.base64 || (el as any).image || '';
    if (!href) return '';
