...
export interface Guideline {
  id: string;
  axis: 'h' | 'v';   // horizontal = spans full width; vertical = spans full height
  position: number;  // SVG viewport-space coordinate
...
export class PrecisionEngine {
  ...
  private minorDivisions: number;
  private showMinorGrid: boolean;
  private pathSnapThreshold: number;
  private unit: 'mm' | 'in' | 'px';

  // ── Ruler canvases ─────────────────────────────────────────────
  private rulerCanvasH: HTMLCanvasElement | null = null;
  private rulerCanvasV: HTMLCanvasElement | null = null;
  private rulerCanvasWrap: HTMLElement | null = null;
  ...
  private guideLayer: SVGSVGElement | null = null;
  private draggingGuide: Guideline | null = null;
  private guideGhostEl: SVGLineElement | null = null;
  private guideGhostAxis: 'h' | 'v' | null = null;
  ...
  private snapToPathEnabled = false;

  // ── Snap indicator ─────────────────────────────────────────────
  private snapIndicator: HTMLElement | null = null;
  private snapIndicatorTimeout: ReturnType<typeof setTimeout> | null = null;

  // ── Panzoom accessor ───────────────────────────────────────────
  private getPanzoomState: (() => { scale: number; x: number; y: number }) | null = null;
  ...
  private _boundRulerHDown: ((e: MouseEvent) => void);
  private _boundRulerVDown: ((e: MouseEvent) => void);
  ...
  public setPanzoomAccessor(fn: () => { scale: number; x: number; y: number }) {
  ...
  public toggleRulers(force?: boolean) {
    ...
    // Sync checkbox
    const cb = document.getElementById('grid-show-rulers') as HTMLInputElement | null;
    if (cb) cb.checked = this.rulersVisible;
  ...
  public toggleGrid(force?: boolean) {
    ...
    if (this.gridCanvas) this.gridCanvas.style.display = this.gridVisible ? 'block' : 'none';
  ...
  public setSnapToPath(enabled: boolean) {
  ...
  public snapPoint(pt: { x: number; y: number }): { x: number; y: number } {
    ...
    if (snapped) this._flashSnapIndicator(result);
    return result;
  }
   ...
   */
  public setGridConfig(majorSize: number, minorDivs: number, showMinor?: boolean) {
    this.majorGridSize  = Math.max(1, majorSize);
    this.minorDivisions = Math.max(1, Math.floor(minorDivs));
    if (showMinor !== undefined) this.showMinorGrid = showMinor;
  ...
  public removeGuideline(id: string) {
  ...
  public addGuideline(axis: 'h' | 'v', position: number): Guideline {
  ...
  private _initRulers() {
    this.rulerCanvasH    = document.getElementById('ruler-h')    as HTMLCanvasElement | null;
    this.rulerCanvasV    = document.getElementById('ruler-v')    as HTMLCanvasElement | null;
    this.rulerCanvasWrap = document.getElementById('ruler-canvas-wrap') as HTMLElement | null;

    this.crosshairH = document.getElementById('ruler-crosshair-h') as unknown as SVGLineElement | null;
    this.crosshairV = document.getElementById('ruler-crosshair-v') as unknown as SVGLineElement | null;
  ...
  private _paintRulerAxis(
    ...
    ctx.lineWidth = 1;
    ctx.beginPath();
    if (axis === 'h') { ctx.moveTo(0, H - 0.5); ctx.lineTo(W, H - 0.5); }
   ...
   */
  private _niceTickInterval(scale: number): { major: number; minor: number } {
    const targetPx = 60;
  ...
  private _formatRulerValue(value: number, step: number): string {
  ...
  private _initGuideLayer() {
    this.guideLayer = document.getElementById('guideline-layer') as SVGSVGElement | null;
  ...
  private _buildGuideline(
    ...
    if (isGhost) line.classList.add('guide-ghost');
    ...
    if (!isGhost) {
      labelEl = document.createElementNS(ns, 'text') as SVGTextElement;
  ...
  private _repositionGuidelines() {
    this.guidelines.forEach(g => {
      if (g.el) this._layoutGuideline(g.el, g.labelEl, g.axis, g.position);
    });
    if (this.guideGhostEl && this.draggingGuide) {
      this._layoutGuideline(
        this.guideGhostEl,
        null,
        this.draggingGuide.axis,
        this.draggingGuide.position
      );
    }
  }
  ...
  private _initGridCanvas() {
    ...
    let canvas = host.querySelector<HTMLCanvasElement>('#grid-canvas');
    if (!canvas) {
  ...
  private _renderGrid() {
    ...
    if (!canvas || !this.gridVisible) return;
    ...
    if (this.showMinorGrid && minorPx >= 4) {
      ...
      for (let x = x0; x <= W; x += minorPx) { ctx.moveTo(x, 0); ctx.lineTo(x, H); }
      for (let y = y0; y <= H; y += minorPx) { ctx.moveTo(0, y); ctx.lineTo(W, y); }
      ctx.stroke();
    }
  ...
  private _snapToGrid(pt: { x: number; y: number }): { x: number; y: number } {
  ...
  private _snapToNearestPath(
    ...
    if (!vpCTM) return null;
    const vpCTMInv = vpCTM.inverse();

    let bestDSq = Infinity;
    let bestPt: { x: number; y: number } | null = null;

    const elements = viewport.querySelectorAll<SVGGeometryElement>(
      'path, rect, circle, ellipse, line, polyline, polygon'
    );

    elements.forEach(el => {
      if (el.getAttribute('data-guide-id')) return; // skip guide overlay elements
      ...
      for (let i = 0; i <= samples; i++) {
        let raw: SVGPoint;
        try { raw = el.getPointAtLength(Math.min(i * step, total)); } catch { continue; }
        ...
        if (dSq < bestDSq) { bestDSq = dSq; bestPt = { x: vx, y: vy }; }
      }
      ...
      for (const c of corners) {
        ...
        const dy = c.y - pt.y;
        const dSq = dx * dx + dy * dy;
        if (dSq < bestDSq) { bestDSq = dSq; bestPt = c; }
      }
    });
  ...
  private _flashSnapIndicator(worldPt: { x: number; y: number }) {
    ...
    if (this.snapIndicatorTimeout) clearTimeout(this.snapIndicatorTimeout);
  ...
  private _bindUIControls() {
    ...
    (document.getElementById('grid-show-rulers') as HTMLInputElement | null)
      ?.addEventListener('change', e => {
        this.toggleRulers((e.target as HTMLInputElement).checked);
      });
    ...
    // ── Grid major spacing ────────────────────────────────────────
    (document.getElementById('grid-major-spacing') as HTMLInputElement | null)
      ?.addEventListener('change', e => {
    ...
    // ── Grid minor spacing ─────────────────────────────────────────
    (document.getElementById('grid-minor-spacing') as HTMLInputElement | null)
      ?.addEventListener('change', e => {
        const minorSize = parseFloat((e.target as HTMLInputElement).value);
    ...
    (document.getElementById('grid-show-minor') as HTMLInputElement | null)
      ?.addEventListener('change', e => {
        this.showMinorGrid = (e.target as HTMLInputElement).checked;
        this._renderGrid();
      });
  ...
  private _onRulerMouseDown(e: MouseEvent, axis: 'h' | 'v') {
    e.preventDefault();
    ...
    // Keep ghost on top
    if (ghost.el) this.guideLayer?.appendChild(ghost.el);
  }
  ...
  private _onGlobalMouseMove(e: MouseEvent) {
    ...
    if (this.draggingGuide.el) {
      this._layoutGuideline(
        this.draggingGuide.el,
        this.draggingGuide.labelEl,
        axis,
        newPos
      );
    }
  ...
  private _onGlobalMouseUp(e: MouseEvent) {
    if (!this.draggingGuide) return;
  ...
  private _updateCrosshairs(e: MouseEvent) {
    const rect = this.container.getBoundingClientRect();
   ...
   */
  private _getViewportState(): { scale: number; tx: number; ty: number } {
    ...
    const mainSvg = this.container.querySelector('svg') as SVGSVGElement | null;
    const viewport = mainSvg?.querySelector('#viewport') as SVGGraphicsElement | null;
    if (!viewport || !mainSvg) return { scale: 1, tx: 0, ty: 0 };

    const vpCtm  = viewport.getScreenCTM();
    const svgCtm = mainSvg.getScreenCTM();
    if (!vpCtm || !svgCtm) return { scale: 1, tx: 0, ty: 0 };
  ...
  private _screenToWorld(screenX: number, screenY: number, axis: 'h' | 'v'): number {
    const rect  = this.container.getBoundingClientRect();
    const state = this._getViewportState();
  ...
  private _restoreSettings() {
    ...
    if (rv === 'false') this.toggleRulers(false);

    const gv = localStorage.getItem('vectronomy_grid_canvas_visible');
    if (gv === 'false') this.toggleGrid(false);

    const sg = localStorage.getItem('vectronomy_snap_grid_v2');
    if (sg === 'true') { this.snapToGridEnabled = true; document.getElementById('btn-toggle-snap')?.classList.add('active'); }

    const sp = localStorage.getItem('vectronomy_snap_path');
    if (sp === 'true') { this.snapToPathEnabled = true; document.getElementById('btn-toggle-path-snap')?.classList.add('active'); }

    const major = localStorage.getItem('vectronomy_major_grid');
    const minor = localStorage.getItem('vectronomy_minor_divs');
    if (major) this.majorGridSize  = Math.max(1, parseFloat(major));
    if (minor) this.minorDivisions = Math.max(1, parseInt(minor));
  }
}
