...
export class MillingEngine {
  ...
  static generatePocketHatch(svg: string, stepover: number, angleDeg: number): string | null {
    ...
    for (let y = startY; y <= endY; y += stepover) {
        ...
        const rect = new paper.Path.Rectangle(
            new paper.Point(expanded.left, y - 0.01),
            new paper.Point(expanded.right, y + 0.01)
        );
        lines.addChild(rect);
    }
    ...
    if (!result || (typeof result.isEmpty === 'function' && result.isEmpty())) return null;
  ...
  static convertToDrill(svgs: string[], maxDiameter: number): { svgs: string[], changed: boolean[] } {
     this.init();
     ...
     for (const svg of svgs) {
         ...
         if (diameter <= maxDiameter && isCircular && areaValid) {
             ...
             const doc = new DOMParser().parseFromString(crosshair.exportSVG({ asString: true }) as string, 'image/svg+xml');
  ...
  static addHoldingTabs(svg: string, tabCount: number, tabWidth: number, tabThickness: number): string | null {
    ...
    for (let i = tabCount - 1; i >= 0; i--) {
        ...
        if (middlePart) {
            ...
            const doc = new DOMParser().parseFromString(middlePart.exportSVG({ asString: true }) as string, 'image/svg+xml');
            const el = doc.documentElement.firstElementChild;
            if (el) {
                ...
                const tabItem = paper.project.importSVG(el.outerHTML, { expandShapes: true }) as paper.Item;
  ...
  static generateConcentricHatch(svg: string, stepover: number): string[] {
    ...
    while (true) {
        ...
        for (const newPath of newPaths) {
            ...
            paper.project.clear();
            const temp = paper.project.importSVG(newPath.outerHTML) as paper.Item;
