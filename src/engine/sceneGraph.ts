...
import type { XcsCanvas, XcsDisplay, XcsGroupData, XcsLayerData, BBox } from './types';
...
export interface ResolvedElement {
  ...
  rootId: string;
  layerTag: string;
  groupChain: string[]; // ancestor group IDs from root to element
}
...
export class SceneGraph {
  private elementMap = new Map<string, XcsDisplay>();
  private groupDataMap = new Map<string, XcsGroupData>();
  private layerDataMap = new Map<string, XcsLayerData>();
  private worldTransforms = new Map<string, Transform>();
  private parentMap = new Map<string, string>(); // childId → parentId
   ...
   */
  private computeAllWorldTransforms(): void {
  ...
  getWorldTransform(id: string): Transform {
    ...
    if (el) {
      ...
      local = Transform.fromXcsElement(groupData as any);
    } else {
      local = new Transform();
    }
  ...
  resolveAll(): ResolvedLayer[] {
    const displays = this.canvas.displays || [];
    const layerMap = new Map<string, ResolvedElement[]>();
  ...
  calculateBoundingBox(): BBox {
    ...
    const expandBounds = (p: { x: number; y: number }) => {
    ...
    for (const el of displays) {
      ...
      if (!hasGeometry || !el.visible) continue;
      ...
      if (el.charJSONs && Array.isArray(el.charJSONs) && el.charJSONs.length > 0) {
        ...
        for (const char of el.charJSONs) {
          ...
          const charWt = wt.clone().multiply(Transform.fromXcsElement(char as any));
