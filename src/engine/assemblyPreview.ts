...
export class AssemblyPreviewEngine {
  ...
  public clear() {
    while (this.assemblyGroup.children.length > 0) {
      const child = this.assemblyGroup.children[0] as THREE.Mesh;
  ...
  public addSvgExtrusions(svgs: string[], thickness: number, materialType: 'wood' | 'acrylic' = 'wood') {
    if (!paper.project) {
  ...
  private extrudeShape(shape: THREE.Shape, depth: number, material: THREE.Material) {
  ...
  private createThreeShapesFromCompoundPath(compoundPath: paper.CompoundPath): THREE.Shape[] {
      const paths = compoundPath.children.filter(c => c instanceof paper.Path) as paper.Path[];
  ...
  private createThreeShapeFromPath(paperPath: paper.Path): THREE.Shape {
      const shape = new THREE.Shape();
      this.buildThreePath(shape, paperPath);
      return shape;
  }

  private createThreePathFromPaperPath(paperPath: paper.Path): THREE.Path {
      const path = new THREE.Path();
      this.buildThreePath(path, paperPath);
      return path;
  }

  private buildThreePath(threePath: THREE.Path | THREE.Shape, paperPath: paper.Path) {
      if (paperPath.segments.length === 0) return;
      ...
      for (let i = 1; i < paperPath.segments.length; i++) {
          ...
          if (prev.handleOut.isZero() && curr.handleIn.isZero()) {
              ...
              threePath.bezierCurveTo(
                  ...
                  curr.point.x + curr.handleIn.x, curr.point.y + curr.handleIn.y,
  ...
  private findPath(item: paper.Item): paper.PathItem | null {
