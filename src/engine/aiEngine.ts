...
export class AIEngine {
  ...
  public async loadModel(progressCallback?: (progress: number, text: string) => void) {
    try {
...
Return ONLY raw JSON.`;
  ...
  private interpretAndDraw(jsonString: string) {
      ...
      switch (cmd.type) {
        case 'Rectangle':
          shape = new paper.Path.Rectangle(
             new paper.Point(cmd.x || 0, cmd.y || 0),
             new paper.Size(cmd.width || 100, cmd.height || 100)
          );
          break;
        case 'Circle':
