...
export class GridManager {
    ...
    snapPoint(pt) {
        ...
        return new paper.Point(Math.round(pt.x / gs) * gs, Math.round(pt.y / gs) * gs);
    }
    initSettings() {
        ...
        const gridCheck = document.getElementById('setting-show-grid');
        if (gridCheck) {
        ...
        const snapCheck = document.getElementById('setting-snap-grid');
        if (snapCheck) {
            snapCheck.checked = this.snapEnabled;
            snapCheck.addEventListener('change', (e) => {
                this.toggleSnap(e.target.checked);
            });
        }
