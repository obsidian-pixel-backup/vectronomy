...
export class FabricationEngine {
    ...
    static applyKerfCompensation(svgs, kerfWidth, direction = 'auto') {
        this.init();
        paper.project.clear();
        const resultSvgs = [];
    ...
    static addLeadInOut(svg, length, type = 'in') {
        this.init();
        paper.project.clear();
    ...
    static addTabs(svg, tabCount, tabWidth) {
        this.init();
        paper.project.clear();
        const item = paper.project.importSVG(svg, { expandShapes: true });
        const path = this.findPath(item);
        ...
        for (let i = tabCount - 1; i >= 0; i--) {
            ...
            if (middlePart)
                middlePart.remove(); // This is the tab (the gap we leave uncut)
        }
    ...
    static async sortAndRouteCuts(svgs) {
        this.init();
        paper.project.clear();
        const items = svgs.map(svg => paper.project.importSVG(svg, { expandShapes: true }));
        ...
        const nodes = paths.map((p, i) => {
            if (!p)
                return { depth: 0, index: i, startX: 0, startY: 0, endX: 0, endY: 0 };
            let d = 0;
        ...
        return new Promise((resolve, reject) => {
            worker.onmessage = (e) => {
                if (e.data.type === 'TSP_RESULT' && e.data.id === id) {
                    ...
                    worker.terminate();
                    const sortedSvgs = finalOrder.map((idx) => svgs[idx]);
    ...
    static addOvercut(svg, overcutLength) {
        this.init();
        paper.project.clear();
        const item = paper.project.importSVG(svg, { expandShapes: true });
        const path = this.findPath(item);
    ...
    static applyPowerRamping(svg, cornerThresholdDeg = 60, rampDist = 5) {
        this.init();
        paper.project.clear();
        const item = paper.project.importSVG(svg, { expandShapes: true });
        const path = this.findPath(item);
        ...
        for (let i = 0; i < path.segments.length; i++) {
            ...
            if (offset < 0.1 || offset > path.length - 0.1)
                continue;
        ...
        for (const frag of fragments) {
            ...
            const fragSvg = frag.exportSVG({ asString: true });
    ...
    static addPerforations(svg, dashLen, gapLen) {
        this.init();
        paper.project.clear();
        const item = paper.project.importSVG(svg, { expandShapes: true });
        const path = this.findPath(item);
