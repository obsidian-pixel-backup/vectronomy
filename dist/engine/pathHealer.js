...
export class PathHealer {
     ...
     */
    static async heal(svgContent, options = {}) {
        const { toleranceMm = PROXIMITY_TOLERANCE_MM, enableUnion = false, // Disabled by default: prevents merging separate components
        enableSimplification = false, // Disabled by default: prevents curving precise gear teeth
        simplifyTolerance = SIMPLIFY_TOLERANCE, } = options;
     ...
     */
    static joinProximityPaths(paths, tolerance) {
        let i = 0;
    ...
    static uniteOverlapping(paths) {
        ...
        while (j < paths.length) {
            ...
            const isP1Closed = (p1 instanceof paper.Path && p1.closed) ||
            ...
            for (let k = j + 1; k < paths.length; k++) {
                ...
                if (!p2 || isRemoved(p2))
                    continue;
                const isP2Closed = (p2 instanceof paper.Path && p2.closed) ||
                ...
                if (p1.bounds.intersects(p2.bounds)) {
                    ...
                    if (!intersection.isEmpty()) {
                        ...
                        if (joined instanceof paper.Path ||
     ...
     */
    static normalizeDPI(value, fromDpi = 72, toDpi = 96) {
        return (value * toDpi) / fromDpi;
    }
}
//# sourceMappingURL=pathHealer.js.map