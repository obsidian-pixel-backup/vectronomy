...
export class FileParser {
    ...
    static async parseFile(file) {
        ...
        if (!jsonData) {
            try {
                ...
                throw new Error('Invalid file format: could not parse as ZIP archive or JSON.');
            }
        }
     ...
     *   { project: { canvas: [...] } }
     */
    static extractCanvases(data) {
        const canvases = data.canvas ||
            data.project?.canvas ||
            [];
        if (!Array.isArray(canvases) || canvases.length === 0) {
            throw new Error('No canvas data found in XCS file. Expected canvas[] array.');
        }
        // Filter out empty canvases
        return canvases.filter((c) => c.displays && Array.isArray(c.displays) && c.displays.length > 0);
    }
}
