...
export class VectorEditor {
    ...
    constructor(container, onUpdate, onSelectionChange, onInteractionStart, onInteractionEnd) {
    ...
    onMouseDown(e) {
        ...
        if (this.activeTool === 'magic-wand') {
            const targetEl = target.closest('[data-xcs-id]');
        ...
        if (handle) {
            ...
            if (el)
                this.origBBox = el.getBoundingClientRect();
            this.onInteractionStart();
            return;
        }
        // Prioritize grouped elements (clusters) over individual paths
        const selectable = target.closest('g[data-xcs-id]') || target.closest('[data-xcs-id]');
        if (selectable) {
    ...
    onMouseMove(e) {
        if (this.isDrawing && this.drawingEl) {
            this.updateDraw(e);
            return;
        }
    ...
    performVectorEraser(eraserPathD, size) {
        const mainSvg = this.container.querySelector('svg');
        const viewport = mainSvg?.querySelector('#viewport') || mainSvg;
        if (!viewport)
            return;
    ...
    deleteActiveNode() {
        if (this.activeNodeIndex === null || !this.nodeEditTarget)
            return;
        const node = this.editingNodes[this.activeNodeIndex];
    ...
    alignTo(mode) {
        ...
        switch (mode) {
            case 'left':
                ...
                break;
            case 'center-h':
                dx = canvasX + canvasW / 2 - tBox.x - tBox.width / 2;
                break;
            case 'right':
