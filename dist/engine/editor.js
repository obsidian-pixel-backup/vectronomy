 ...
 */
import { parseSvgPath, absolutizePath, stringifySvgPath, extractNodes } from './pathUtils';
...
import { NestingEngine } from './nesting';
...
export class VectorEditor {
    enterIsolation(groupId) {
        ...
        // Dim everything else
        Array.from((this.container.querySelector('#viewport') || this.container.querySelector('svg'))?.children || []).forEach((child) => {
        ...
        if (breadcrumb) {
            ...
            if (targetName)
                targetName.innerText = (group.getAttribute('data-name') || 'Group ' + groupId.substring(0, 8));
        }
    ...
    constructor(container, onUpdate, onSelectionChange, onInteractionStart, onInteractionEnd, onDrawingChange) {
        ...
        this.rasterEngine = new RasterEngine();
        this.handleSnapEnabled = false;
        // Custom Shapes state
        this.polygonSides = 5;
        this.starPoints = 5;
        this.pencilPoints = [];
        ...
        this.currentStrokeColor = '#00ffc2';
        this.magicWandThreshold = 20;
        this.brushPoints = [];
        this.eraserPoints = [];
        this.brushCursor = null;
        this.snapFn = null;
        // Feature 56 & 60 properties
        this.centroidSnapEnabled = false;
        this.transformPivot = 'cc';
        ...
        this.centroidGuideH = null;
        this.centroidGuideV = null;
        this.eraserCache = new Map();
    ...
    setLayer(layer) { this.currentLayer = layer; this.clearSelection(); }
    setTool(tool) {
        if (this.activeTool === 'pen' && this.isDrawing) {
            this.finalizePenPath();
        }
    ...
    async pathfinderOperation(operation) {
        if (operation === 'divide') {
            return this.pathfinderDivide();
        }
        const els = this.getSelectedEls();
        if (els.length < 2)
            return; // Need at least two shapes
        ...
        for (let i = 1; i < els.length; i++) {
            ...
            if (resultSvgString) {
                ...
                const parser = new DOMParser();
                const doc = parser.parseFromString(resultSvgString, 'image/svg+xml');
                let newEl = doc.documentElement.firstElementChild;
                ...
                if (newEl && newEl.tagName.toLowerCase() !== 'svg') {
                    ...
                    baseEl = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    ...
    strokeToPath() {
        this.processPathfinderUtility((svg, el) => {
    ...
    offsetSelectedPaths(offset) {
        this.processPathfinderUtility((svg) => Pathfinder.offsetPath(svg, offset, 'round'));
    }
    flattenSelectedPaths(tolerance) {
        this.processPathfinderUtility((svg) => Pathfinder.flattenPath(svg, tolerance));
    }
    reverseSelectedPaths() {
        this.processPathfinderUtility((svg) => Pathfinder.reversePath(svg));
    }
    processPathfinderUtility(processor, styleCallback) {
        const els = this.getSelectedEls();
        if (els.length === 0)
            return;
        let changed = false;
    ...
    applyKerf(kerfWidth, direction = 'auto') {
        const els = this.getSelectedEls();
        if (els.length === 0)
            return;
        const svgs = els.map(el => el.outerHTML);
        ...
        for (let i = 0; i < els.length; i++) {
            if (resultSvgs[i] !== svgs[i]) {
                ...
                const newPath = doc.documentElement.firstElementChild;
                if (newPath) {
    ...
    applyLeadInOut(length, type) {
        this.processPathfinderUtility((svg) => FabricationEngine.addLeadInOut(svg, length, type));
    }
    applyMicroJoints(count, width) {
        this.processPathfinderUtility((svg) => FabricationEngine.addTabs(svg, count, width));
    }
    // ── Batch 5 Features ──────────────────────────────────────────
    applyAcrylicOvercut(length) {
        this.processPathfinderUtility((svg) => FabricationEngine.addOvercut(svg, length));
    }
    applyPowerRamping(thresholdDeg = 60) {
        const els = this.getSelectedEls();
        if (els.length === 0)
            return;
        let changed = false;
    ...
    applyPerforations(dashLen, gapLen) {
        this.processPathfinderUtility((svg) => FabricationEngine.addPerforations(svg, dashLen, gapLen));
    }
    ...
    applyMultiPass(passes) {
        const els = this.getSelectedEls();
        if (els.length === 0 || passes <= 1)
            return;
        let changed = false;
        for (const el of els) {
    ...
    async optimizeCutOrder() {
        const mainSvg = this.container.querySelector('svg');
        const viewport = mainSvg?.querySelector('#viewport') || mainSvg;
        if (!viewport)
            return;
        const els = Array.from(viewport.querySelectorAll('path, rect, circle, ellipse, polygon, polyline, line'));
        if (els.length < 2)
            return;
        const svgs = els.map(el => el.outerHTML);
    ...
    async generate3DAssembly(containerId, thickness, materialType = 'wood') {
        const els = this.getSelectedEls();
    ...
    applyCncBitOffset(bitRadius) {
        const els = this.getSelectedEls();
        if (els.length === 0 || bitRadius === 0)
            return;
        const svgs = els.map(el => el.outerHTML);
    ...
    applyPocketHatch(stepover, angleDeg) {
        this.processPathfinderUtility((svg) => MillingEngine.generatePocketHatch(svg, stepover, angleDeg));
    }
    applyChamfer(depth, angleDeg, passes) {
        const els = this.getSelectedEls();
        if (els.length === 0)
            return;
        let changed = false;
    ...
    applyVCarve(bitAngle) {
        this.processPathfinderUtility((svg) => MillingEngine.applyVCarve(svg, bitAngle));
    }
    // ── Batch 7 Features (Final CNC) ──────────────────────────────
    applyMillingHoldingTabs(tabCount, tabWidth, tabThickness) {
        this.processPathfinderUtility((svg) => MillingEngine.addHoldingTabs(svg, tabCount, tabWidth, tabThickness));
    }
    applyConcentricHatch(stepover) {
        const els = this.getSelectedEls();
        if (els.length === 0)
            return;
        let changed = false;
    ...
    applyFeedPlungeRates(feedXY, plungeZ) {
        const els = this.getSelectedEls();
        if (els.length === 0)
            return;
        let changed = false;
        for (const el of els) {
            if (feedXY > 0)
                el.setAttribute('data-cnc-feedrate', feedXY.toString());
            if (plungeZ > 0)
                el.setAttribute('data-cnc-plungerate', plungeZ.toString());
            changed = true;
        }
        if (changed)
            this.commit();
    }
    applyStepoverOverlapConfig(overlapPercent) {
        const els = this.getSelectedEls();
        if (els.length === 0)
            return;
        let changed = false;
    ...
    async traceCenterline(cornerThreshold = 1.5) {
        const els = this.getSelectedEls();
        if (els.length === 0 || els[0].tagName.toLowerCase() !== 'image')
            return;
    ...
    async traceSilhouette(threshold, cornerThreshold = 1.5) {
        const els = this.getSelectedEls();
        if (els.length === 0 || els[0].tagName.toLowerCase() !== 'image')
            return;
        const href = els[0].getAttribute('href');
    ...
    async tracePosterized(colors, cornerThreshold = 1.5) {
        const els = this.getSelectedEls();
        if (els.length === 0 || els[0].tagName.toLowerCase() !== 'image')
            return;
        const href = els[0].getAttribute('href');
    ...
    async traceEdges(threshold, cornerThreshold = 1.5) {
        const els = this.getSelectedEls();
        if (els.length === 0 || els[0].tagName.toLowerCase() !== 'image')
            return;
        const href = els[0].getAttribute('href');
    ...
    async traceColorIsolation(colorHex, tolerance = 30, cornerThreshold = 1.5) {
        const els = this.getSelectedEls();
        if (els.length === 0 || els[0].tagName.toLowerCase() !== 'image')
            return;
        const href = els[0].getAttribute('href');
    ...
    async applyHalftone(spacing, angle) {
        const els = this.getSelectedEls();
        if (els.length === 0 || els[0].tagName.toLowerCase() !== 'image')
            return;
        const href = els[0].getAttribute('href');
    ...
    applyDespeckle(minArea) {
        this.processPathfinderUtility((svg) => TracingEngine.despeckle(svg, minArea));
    }
    replaceImageWithVectors(imgEl, newSvgStr) {
    ...
    joinOpenEndpoints(threshold = 5) {
        ...
        if (mergedAny) {
            ...
            pathInfos.forEach(p => {
                if (!p.merged)
                    this.selectedIds.add(p.el.getAttribute('data-xcs-id'));
            });
    ...
    applyImageFilters(brightness, contrast, blur) {
        const els = this.getSelectedEls();
        if (els.length !== 1)
            return;
        const imgEl = els[0];
    ...
    async convertTextToPath(textEl) {
        if (!textEl) {
            const els = this.getSelectedEls();
            textEl = els.find(el => el.tagName.toLowerCase() === 'text');
        }
        if (!textEl)
            return;
        ...
        if (group.children.length === 0)
            return undefined; // tracing failed somehow
    ...
    async warpTextToPath(textEl, targetPath) {
        if (!textEl || !targetPath) {
            const els = this.getSelectedEls();
            textEl = els.find(el => el.tagName.toLowerCase() === 'text');
            targetPath = els.find(el => el.tagName.toLowerCase() === 'path' || el.tagName.toLowerCase() === 'polyline');
        }
        if (!textEl || !targetPath)
            return;
        ...
        try {
            pathTotalLength = targetPath.getTotalLength();
        }
        catch {
            return;
        }
        if (pathTotalLength === 0)
            return;
        const letters = Array.from(group.children);
        ...
        letterData.forEach(data => {
            ...
            if (lengthAlongPath > pathTotalLength)
                return;
    ...
    injectStencilBridges(bridgeWidth = 2) {
        const els = this.getSelectedEls();
        if (els.length === 0)
            return;
        const svgs = els.map(el => el.outerHTML);
        ...
        const parser = new DOMParser();
        const doc = parser.parseFromString(resultSvg, 'image/svg+xml');
        const resultElement = doc.querySelector('svg')?.firstElementChild;
        if (resultElement) {
    ...
    onMouseDown(e) {
        ...
        if (this.activeTool === 'magic-wand') {
            const targetEl = target.closest('[data-xcs-id]:not(.vectronomy-frame)') || target.closest('[data-xcs-id]');
            if (targetEl) {
                ...
                if (allElements) {
                    allElements.forEach(el => {
                        if (el.tagName.toLowerCase() === 'g' && el.classList.contains('vectronomy-frame'))
                            return;
                        if (el.classList.contains('isolated-backdrop'))
                            return;
        ...
        if (this.activeTool === 'node') {
            ...
            const nodeHandle = target.closest('[data-node-index]');
            if (nodeHandle) {
                e.stopPropagation();
                e.preventDefault();
            ...
            if (!clickedPath) {
                const primitive = target.closest('rect, circle, ellipse, line, polygon, polyline');
        ...
        const selectable = target.closest('g[data-xcs-id]:not(.vectronomy-frame)') || target.closest('[data-xcs-id]');
        if (selectable) {
            ...
            const closestPath = target.closest('path');
            if (closestPath) {
    ...
    async convertSelectedToPath() {
        ...
        for (const el of els) {
            ...
            if (tag === 'text') {
                const group = await this.convertTextToPath(el);
                if (group)
                    changed = true;
            }
    ...
    convertToPath(el) {
        ...
        if (tag === 'rect') {
            ...
            const pairs = points.trim().split(/[\s,]+/).reduce((result, value, index, array) => {
                if (index % 2 === 0)
    ...
    onMouseMove(e) {
        ...
        if (this.isDrawing && (this.drawingEl || this.activeTool === 'eraser')) {
            this.updateDraw(e);
            return;
        }
    ...
    clearCentroidGuides() {
        if (this.centroidGuideH) {
            this.centroidGuideH.remove();
            this.centroidGuideH = null;
        }
        if (this.centroidGuideV) {
            this.centroidGuideV.remove();
            this.centroidGuideV = null;
        }
    }
    drawCentroidGuides(els) {
        if (els.length === 0)
            return;
        ...
        for (const other of Array.from(allEls)) {
            if (els.includes(other))
                continue;
            const otherBox = this.getTransformedBBox(other);
            if (!otherBox)
                continue;
        ...
        if (!snapX && this.centroidGuideV) {
            this.centroidGuideV.remove();
            this.centroidGuideV = null;
        }
        if (!snapY && this.centroidGuideH) {
            this.centroidGuideH.remove();
            this.centroidGuideH = null;
        }
    }
    onMouseUp() {
        ...
        if (this.isMarquee && this.marqueeEl) {
            ...
            if (mainSvg) {
                ...
                allEls.forEach(el => {
                    if (el.tagName.toLowerCase() === 'path' && el.parentElement?.hasAttribute('data-xcs-id'))
                        return;
    ...
    updateDraw(e) {
        if (!this.drawingEl && this.activeTool !== 'eraser')
            return;
        if (this.activeTool === 'pen') {
            const pt = this.getSvgPoint(e);
            if (!pt)
    ...
    finalizeDraw() {
        if (this.activeTool === 'pen' || this.activeTool === 'polyline')
            return;
        ...
        if (this.activeTool === 'frame' && this.drawingEl) {
            const mainSvg = this.container.querySelector('svg');
            const viewport = mainSvg?.querySelector('#viewport') || mainSvg;
            const frameRect = this.drawingEl;
    ...
    performVectorEraser(points, size) {
        const mainSvg = this.container.querySelector('svg');
        const viewport = mainSvg?.querySelector('#viewport') || mainSvg;
        if (!viewport)
            return;
        ...
        elements.forEach(el => {
            if (['path', 'rect', 'circle', 'ellipse', 'polygon', 'polyline', 'line'].includes(el.tagName.toLowerCase())) {
                ...
                if (!cached) {
                    const elBox = this.getBBoxInViewport(el, viewport);
                    ...
                    if (newPaths.length > 0) {
                        ...
                        if (newPaths.length === 1 && (!newPaths[0].getAttribute('d') || newPaths[0].getAttribute('d').length < 2)) {
                            ...
                            newPaths.forEach((np, i) => {
                                ...
                                if (originalStroke)
                                    np.setAttribute('stroke', originalStroke);
                                if (originalFill)
                                    np.setAttribute('fill', originalFill);
                                if (originalStrokeWidth)
                                    np.setAttribute('stroke-width', originalStrokeWidth);
                                if (i === 0 && originalId)
                                    np.setAttribute('data-xcs-id', originalId);
                                else
                                    np.setAttribute('data-xcs-id', `el-${Date.now()}-${Math.floor(Math.random() * 1000)}`);
                                fragment.appendChild(np);
                            });
    ...
    createGridArray(rows, cols, spacingX, spacingY) {
    ...
    canUndoDrawing() {
        ...
        if (this.activeTool === 'pen' && this.penPoints.length > 0)
            return true;
        if (this.activeTool === 'polyline' && this.polylinePoints.length > 0)
            return true;
        return false;
    }
    undoDrawing() {
    ...
    renderPenOverlay(ptMouse) {
        const mainSvg = this.container.querySelector('svg');
        if (!mainSvg)
            return;
        let overlay = mainSvg.querySelector('.pen-overlay');
        if (!overlay) {
    ...
    getPathD(points, closed = false, previewPoint) {
        if (points.length === 0)
            return '';
    ...
    getSvgPoint(e) {
        ...
        let ctm = viewport.getScreenCTM();
        if (!ctm)
            ctm = mainSvg.getScreenCTM();
        if (!ctm)
            return pt; // Last resort fallback
    ...
    getSelectedEls() {
        const mainSvg = this.container.querySelector('svg');
        if (!mainSvg || this.selectedIds.size === 0)
            return [];
        const els = [];
        this.selectedIds.forEach(id => {
            const el = mainSvg.querySelector(`[data-xcs-id="${id}"]`);
            if (el)
                els.push(el);
        });
        return els;
    ...
    deleteActiveNode() {
        if (this.selectedNodeIndices.size === 0 || !this.nodeEditTarget)
            return;
    ...
    renderNodeUI(el) {
        ...
        if (!this.draggingNode) {
            ...
            const vpScreenCTM = viewport.getScreenCTM();
            const invScale = 1 / (vpScreenCTM.a || 1);
            const hs = 4 * invScale;
            const pathCTM = el.getCTM();
            const vpCTM = viewport.getCTM();
            let pathToViewport = null;
            if (pathCTM && vpCTM) {
                pathToViewport = vpCTM.inverse().multiply(pathCTM);
            }
            const g = this.selectionGroup.firstChild;
            if (!g)
                return;
    ...
    resizeSelection(dx, dy, handle, shiftKey = false, altKey = false) {
        const els = this.getSelectedEls();
        if (els.length === 0)
            return;
        const tBox = this.getUnionBBox(els);
        ...
        if (handle.includes('r'))
            sx = Math.max(0.001, tBox.width + adx) / Math.max(tBox.width, 0.001);
        if (handle.includes('l'))
            sx = Math.max(0.001, tBox.width - adx) / Math.max(tBox.width, 0.001);
        if (handle.includes('b'))
            sy = Math.max(0.001, tBox.height + ady) / Math.max(tBox.height, 0.001);
        ...
        if (altKey) {
            ...
            if (handle.includes('l'))
                pivotX = tBox.x + tBox.width;
            if (handle.includes('t'))
                pivotY = tBox.y + tBox.height;
        ...
        els.forEach((el) => {
            if (el.classList.contains('vectronomy-frame')) {
                ...
                if (bgRect && clipRect) {
                    ...
                    const pathCTM = el.getCTM();
                    const vpCTM = (mainSvg.querySelector('#viewport') || mainSvg).getCTM();
    ...
    rotateSelection(e, shiftKey) {
        ...
        const pt1 = this.getSvgPoint({ clientX: this.dragStartX, clientY: this.dragStartY });
        const pt2 = this.getSvgPoint({ clientX, clientY });
        if (!pt1 || !pt2)
            return;
    ...
    resizeEl(el, dx, dy, handle) {
        ...
        if (tag === 'rect') {
            ...
            if (handle.includes('r') || handle.includes('l'))
                rx = Math.max(0.1, rx + Math.abs(dx) * Math.sign(dx));
            if (handle.includes('b') || handle.includes('t'))
                ry = Math.max(0.1, ry + Math.abs(dy) * Math.sign(dy));
            ...
            let sy = 1;
            if (handle.includes('r'))
                sx = Math.max(0.001, tBox.width + dx) / Math.max(tBox.width, 0.001);
            if (handle.includes('l'))
                sx = Math.max(0.001, tBox.width - dx) / Math.max(tBox.width, 0.001);
            if (handle.includes('b'))
                sy = Math.max(0.001, tBox.height + dy) / Math.max(tBox.height, 0.001);
            if (handle.includes('t'))
                sy = Math.max(0.001, tBox.height - dy) / Math.max(tBox.height, 0.001);
            let pivotX = tBox.x;
            let pivotY = tBox.y;
    ...
    // ── Notify ────────────────────────────────────────────────────
    getPivotCoords(uBox, pivot) {
        let px = uBox.x;
        let py = uBox.y;
        if (pivot.endsWith('c'))
            px = uBox.x + uBox.width / 2;
        if (pivot.endsWith('r'))
            px = uBox.x + uBox.width;
        if (pivot.startsWith('c'))
            py = uBox.y + uBox.height / 2;
        if (pivot.startsWith('b'))
            py = uBox.y + uBox.height;
        return { x: px, y: py };
    }
    buildElementProperties(el) {
        const els = this.getSelectedEls();
        const tBox = this.getUnionBBox(els);
    ...
    getMultiSelectionProperties() {
        const els = this.getSelectedEls();
        if (els.length === 0)
            return null;
        if (els.length === 1)
            return this.buildElementProperties(els[0]);
        const tBox = this.getUnionBBox(els);
    ...
    updateProperties(props) {
        ...
        if (props.opacity !== undefined)
            setAttr('opacity', `${(props.opacity / 100).toFixed(2)}`);
        if (props.strokeColor !== undefined)
            setAttr('stroke', props.strokeColor);
        if (props.strokeW !== undefined)
            setAttr('stroke-width', `${props.strokeW}`);
        if (props.strokeCap !== undefined)
            setAttr('stroke-linecap', props.strokeCap);
        if (props.strokeJoin !== undefined)
            setAttr('stroke-linejoin', props.strokeJoin);
        if (props.strokeOpacity !== undefined)
            setAttr('stroke-opacity', `${(props.strokeOpacity / 100).toFixed(2)}`);
        if (props.fillEnabled !== undefined || props.fillColor !== undefined || props.fillOpacity !== undefined) {
            ...
            if (props.fillOpacity !== undefined)
                setAttr('fill-opacity', `${(props.fillOpacity / 100).toFixed(2)}`);
            if (props.fillRule !== undefined)
                setAttr('fill-rule', props.fillRule);
        }
        ...
        if (props.fontStyle !== undefined)
            setAttr('font-style', props.fontStyle);
        if (props.letterSpacing !== undefined)
            setAttr('letter-spacing', `${props.letterSpacing}`);
        if (props.wordSpacing !== undefined)
            setAttr('word-spacing', `${props.wordSpacing}`);
        if (props.textAnchor !== undefined)
            setAttr('text-anchor', props.textAnchor);
        this.renderSelectionUI();
        this.commit();
        this.notifyChange(els[0] || null);
    }
    ...
    bringForward() {
        const el = this.getSelectedEl();
        if (!el || !el.nextElementSibling || !el.parentNode)
            return;
    ...
    sendBackward() {
        const el = this.getSelectedEl();
        if (!el || !el.previousElementSibling || !el.parentNode)
            return;
    ...
    alignTo(mode) {
        ...
        switch (mode) {
            case 'left':
                ...
                break;
            case 'center-h':
                dx = canvasX + canvasW / 2 - pivotCoords.x;
                break;
            case 'right':
                dx = canvasX + canvasW - (tBox.x + tBox.width);
                break;
            case 'top':
                dy = canvasY - tBox.y;
                break;
            case 'center-v':
                dy = canvasY + canvasH / 2 - tBox.y - tBox.height / 2;
                break;
            case 'bottom':
                dy = canvasY + canvasH - tBox.y - tBox.height;
                break;
        }
        els.forEach(el => this.translateEl(el, dx, dy));
        this.renderSelectionUI();
    ...
    alignSelectedNodes(alignment) {
        if (this.selectedNodeIndices.size < 2 || !this.nodeEditTarget)
            return;
    ...
    insertImage(dataUrl, x, y, width, height) {
        const mainSvg = this.container.querySelector('svg');
        const viewport = mainSvg?.querySelector('#viewport') || mainSvg;
        if (!viewport)
            return;
