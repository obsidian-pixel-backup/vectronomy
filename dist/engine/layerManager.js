export class LayerManager {
    constructor(editor) {
        ...
        this.editor = editor;
        this.container = document.getElementById('props-layers');
    ...
    updateTree() {
        ...
        const buildNode = (el, parentEl) => {
            // Skip internal ui elements
            if (el.classList && (el.classList.contains('selection-overlay') || el.classList.contains('canvas-ui')))
                return;
            if (el.tagName.toLowerCase() === 'defs' || el.tagName.toLowerCase() === 'style')
                return;
            ...
            if (isGroup && el.children.length > 0) {
                ...
                expander.addEventListener('click', (e) => {
                    ...
                    const childrenDiv = itemDiv.querySelector('.layer-children');
                    if (childrenDiv)
            ...
            rowDiv.addEventListener('click', (e) => {
                e.stopPropagation();
                if (isLocked)
                    return; // Prevent selecting locked layers? Or just allow selecting in tree but not canvas?
