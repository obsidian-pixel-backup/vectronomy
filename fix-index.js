...
// Verify that the lines we are removing start with "              <h4>The Node Editor" just to be safe
if (lines[1135].includes("The Node Editor") && lines[1138].includes("</ul>")) {
  lines.splice(1135, 4, ...newBlock);
  fs.writeFileSync('e:/DEVELOPER PROJECTS/vectronomy/index.html', lines.join('\n'));
  console.log("Successfully replaced lines.");
} else {
  console.log("Target lines do not match expectations. Check indices.");
}
